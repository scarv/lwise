...
- Build and execute implementation, e.g.,
  ...
  `${REPO_HOME}/bin/test.py` 
  which automates an scan through various different configurations, e.g., 
...
- `RV32_TYPE1`: base ISA.
  ...
  - synthesise rotate via, e.g., per [Sec. 5.2.3, 2]
...
- `RV64_TYPE1`: base ISA.
  ...
  - ironically this is a bit tricky, because the (internal) operations are 
    on 32-bit sub-words
  - the RISC-V P(acked) extension doesn't seem mature enough to consider,
    and it's debatable whether they'd include the operations required; it
    *might* be possible to consider wrt. the XCrypto packed feature

...
#define measure_prologue(id)                                                       \
  uint32_t id ## _cycle_b =  0;                                                    \
  uint32_t id ## _cycle_a =  0;                                                    \
  uint32_t id ## _cycle_t =  0;                                                    \
                                                                                   \
  uint32_t id ##  _iret_b =  0;                                                    \
  uint32_t id ##  _iret_a =  0;                                                    \
  uint32_t id ##  _iret_t =  0;
...
#define measure_prologue(id)                                                       \
  uint32_t id ## _cycle_b =  0;                                                    \
  uint32_t id ## _cycle_a =  0;                                                    \
  uint32_t id ## _cycle_t = -1;                                                    \
                                                                                   \
  uint32_t id ##  _iret_b =  0;                                                    \
  uint32_t id ##  _iret_a =  0;                                                    \
  uint32_t id ##  _iret_t = -1;
#define measure_step(id,...)                                                       \
  COUNTERS(id ## _cycle_b, id ## _iret_b);                                         \
  id( __VA_ARGS__ );                                                               \
  COUNTERS(id ## _cycle_a, id ## _iret_a);                                         \

...
#define AEAD_MAX_MESSAGE_LENGTH			32
#define AEAD_MAX_ASSOCIATED_DATA_LENGTH	32
#define HASH_MAX_MESSAGE_LENGTH			1024
...
int aead_generate_test_vectors()
{
	...
	unsigned char       msg[AEAD_MAX_MESSAGE_LENGTH];
	unsigned char       msg2[AEAD_MAX_MESSAGE_LENGTH];
	unsigned char		ad[AEAD_MAX_ASSOCIATED_DATA_LENGTH];
	unsigned char		ct[AEAD_MAX_MESSAGE_LENGTH + CRYPTO_ABYTES];
	...
	for (unsigned long long mlen = 0; (mlen <= AEAD_MAX_MESSAGE_LENGTH) && (ret_val == KAT_SUCCESS); mlen++) {

		for (unsigned long long adlen = 0; adlen <= AEAD_MAX_ASSOCIATED_DATA_LENGTH; adlen++) {
...
int hash_generate_test_vectors()
{
	FILE                *fp;
	char                fileName[MAX_FILE_NAME];
	unsigned char       msg[HASH_MAX_MESSAGE_LENGTH];
	...
	for (unsigned long long mlen = 0; mlen <= HASH_MAX_MESSAGE_LENGTH; mlen++) {
